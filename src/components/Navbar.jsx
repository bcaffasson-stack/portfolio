        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <div className="md:hidden flex items-center space-x-3">
              <button onClick={toggleDarkMode} className="p-2 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 transition-all duration-300">
                <i className={`fas ${darkMode ? 'fa-sun' : 'fa-moon'} text-lg`} />
              </button>
              <button
            ...
            >
              <div className="p-6 pt-8">
                <div className="flex items-center gap-4 mb-8 p-4 bg-gray-50 dark:bg-gray-800 rounded-2xl">
                  ...
                  <div>
                    <div className="font-semibold text-gray-900 dark:text-white">{personalInfo.shortName}</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">{personalInfo.title}</div>
                  </div>
                </div>
                  ...
                  {navLinks.map((link, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {link.label}
                        {isActive && <motion.div layoutId="mobileActive" className="ml-auto w-1.5 h-1.5 bg-primary-500 rounded-full" />}
                      </motion.a>
                    );
                  })}
                </div>
                ...
                >
                  <a href={`mailto:${personalInfo.email}`} className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400 hover:text-primary-500 transition-colors">
                    <i className="fas fa-envelope w-4" /><span className="truncate">{personalInfo.email}</span>
                  </a>
                  <a href={`tel:${personalInfo.phone.replace(/\s/g, '')}`} className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400 hover:text-primary-500 transition-colors">
                    <i className="fas fa-phone w-4" /><span>{personalInfo.phone}</span>
                  </a>
                  <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
                    <i className="fas fa-location-dot w-4" /><span>{personalInfo.location}</span>
                  </div>
                </motion.div>
              </div>
            </motion.div>
